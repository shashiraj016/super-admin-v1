...
export class DashboardComponent implements AfterViewInit, OnInit {
  ...
  apiUrl: string = 'https://dev.smartassistapp.in/api/superAdmin/dashbaordNew';
  ...
  fetchDealerSMs(
    ...
    const url = `https://dev.smartassistapp.in/api/superAdmin/dashboard/NoSM?dealer_id=${dealerId}&type=${type}`;
  ...
  fetchPS(smId: string, dealerId: string) {
    ...
    const headers = new HttpHeaders({ Authorization: `Bearer ${token}` });
    const url = `https://dev.smartassistapp.in/api/superAdmin/dashbaordNew?type=${this.selectedFilter}&dealer_id=${dealerId}&sm_id=${smId}`;
  ...
  loadKpiData(type: string = 'DAY') {
    ...
    this.http
      .get<any>(
        `https://dev.smartassistapp.in/api/superAdmin/dashboard/NoSM?type=${type}`,
        { headers }
      )
      .subscribe({
        next: (res) => {
  ...
  fetchSuperAdminDashboard(type: string): void {
    ...
    if (!this.selectedDealers?.length) {
      ...
      url = isCustomMode
        ...
        : `https://dev.smartassistapp.in/api/superAdmin/dashboard/NoSM?dealer_id=${this.selectedDealers[0].dealerId}&type=${type}`;
    } else {
      const dealerIds = this.selectedDealers.map((d) => d.dealerId).join(',');
      url = isCustomMode
        ? `https://dev.smartassistapp.in/api/superAdmin/dashboard/NoSM?dealerIds=${dealerIds}&start_date=${this.customStartDate}&end_date=${this.customEndDate}`
        : `https://dev.smartassistapp.in/api/superAdmin/dashboard/NoSM?dealerIds=${dealerIds}&type=${type}`;
    }
  ...
  fetchDealers(filter: 'MTD' | 'QTD' | 'YTD') {
    ...
    const url = `https://dev.smartassistapp.in/api/superAdmin/dashboard/view-activities?type=${filter}`;
  ...
  onDealerClick(dealerId: string, type: string) {
    ...
    if (
      ...
      const url = `https://dev.smartassistapp.in/api/superAdmin/dashboard/NoSM?dealer_id=${dealerId}&start_date=${this.customStartDate}&end_date=${this.customEndDate}`;
  ...
  fetchPS1Data(): void {
    ...
    const baseUrl = 'https://dev.smartassistapp.in/api/superAdmin/dashbaordNew';
  ...
  fetchDashboardData(type: 'MTD' | 'QTD' | 'YTD' = 'MTD') {
    ...
    const url = `https://dev.smartassistapp.in/api/superAdmin/dashboard/view-activities?type=${type}`;
  ...
  fetchData(): void {
    this.http
      .get<any>('https://dev.smartassistapp.in/api/superAdmin/dashbaordNew')
      .subscribe(
        (response) => {
  ...
  fetchDashboardDataForTopCards(filter: string) {
    const token = localStorage.getItem('token') || '';
    let url = `https://dev.smartassistapp.in/api/superAdmin/dashboard/NoSM`;
